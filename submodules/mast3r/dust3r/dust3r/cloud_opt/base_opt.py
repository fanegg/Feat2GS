...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        pred1_feat = pred1['feat']
        pred2_feat = pred2['feat']
        self.feat_i = NoGradParamDict({ij: nn.Parameter(pred1_feat[n], requires_grad=False) for n, ij in enumerate(self.str_edges)})
        self.feat_j = NoGradParamDict({ij: nn.Parameter(pred2_feat[n], requires_grad=False) for n, ij in enumerate(self.str_edges)})
