...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, **kwargs):
        ...
        stacked_feat_i = [self.feat_i[k] for k in self.str_edges]
        stacked_feat = [None] * len(self.imshapes)
        for i, ei in enumerate(self._ei):
            stacked_feat[ei]=stacked_feat_i[i]
        stacked_feat = torch.stack(stacked_feat).float().detach()
        self.register_buffer('stacked_feat', stacked_feat)
    ...
    def get_planes3d(self, raw=False):
        ...
        plane_depth = torch.ones_like(depth) * depth.mean(dim=1,keepdim=True)
        # get pointmaps in camera frame
        rel_ptmaps = _fast_depthmap_to_pts3d(plane_depth, self._grid, focals, pp=pp)
        # project to world frame
        res = geotrf(im_poses, rel_ptmaps)
