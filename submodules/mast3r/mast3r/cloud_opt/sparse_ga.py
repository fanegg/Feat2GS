...
from dust3r.utils.image import reshape_feature

class SparseGA():
    def __init__(self, img_paths, pairs_in, res_fine, anchors, canonical_paths=None, featmaps=None):
        def fetch_img(im):
        ...
        self.stacked_feat = torch.stack(featmaps)
...
def sparse_global_alignment(imgs, pairs_in, cache_path, model, subsample=8, desc_conf='desc_conf',
    ...
    tmp_pairs, pairwise_scores, canonical_views, canonical_paths, preds_21, featmaps = \
    ...
    return SparseGA(imgs, pairs_in, res_fine or res_coarse, anchors, canonical_paths, featmaps)
...
def forward_mast3r(pairs, model, cache_path, desc_conf='desc_conf',
    ...
    for img1, img2 in tqdm(pairs):
        ...
        if not all(os.path.isfile(p) for p in (path1, path2, path_corres)):
            ...
            F11, _, F22, _ = [r['feat'][0] for r in res]
            C11, C21, C22, C12 = [r['conf'][0] for r in res]
            descs = [r['desc'][0] for r in res]
            qonfs = [r[desc_conf][0] for r in res]

            # save
            torch.save(to_cpu((X11, C11, F11, X21, C21)), mkdir_for(path1))
            torch.save(to_cpu((X22, C22, F22, X12, C12)), mkdir_for(path2))
...
def symmetric_inference(model, img1, img2, device):
    ...
    # Resize the feature
    feat1 = reshape_feature(feat1, shape1)
    feat2 = reshape_feature(feat2, shape2)

    res11['feat'], res12['feat'], res22['feat'], res21['feat'] = feat1, feat1, feat2, feat2
...
def prepare_canonical_data(imgs, tmp_pairs, subsample, order_imgs=False, min_conf_thr=0,
    ...
    for img in tqdm(imgs):
        ...
        for (img1, img2), ((path1, path2), path_corres) in tmp_pairs.items():
            score = None
            if img == img1:
                X, C, F, X2, C2 = torch.load(path1, map_location=device)
            ...
            if img == img2:
                X, C, F, X2, C2 = torch.load(path2, map_location=device)
                score, (xy1, xy2, confs) = load_corres(path_corres, device, min_conf_thr)
        ...
        featmaps.append(canon_feat)

    return tmp_pairs, pairwise_scores, canonical_views, canonical_paths, preds_21, featmaps
