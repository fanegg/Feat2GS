...
sys.path.insert(0, "/home/chenyue/.cache/torch/hub/zero123/zero123/")
...
@torch.no_grad()
def sample_model(input_im, model, sampler, n_samples=1, scale=3, up_ft_index=[0, 1, 2, 3]):
    ...
    with model.ema_scope():
        ...
        ms_feats = sampler.model.model.diffusion_model(
            xc, t_in, context=cc, return_feats=True,up_ft_index=up_ft_index
        )
...
def get_zero123():
    ckpt = "/home/chenyue/.cache/torch/hub/zero123/zero123/105000.ckpt"
    config = "/home/chenyue/.cache/torch/hub/zero123/zero123/configs/sd-objaverse-finetune-c_concat-256.yaml"
    device = "cuda:0"
...
class Zero123(torch.nn.Module):
    def __init__(self, time_step=1, output="dense", layer=1, return_multilayer=False):
        ...
        self.up_ft_index = [2, 5, 8, 11]  # keep all the upblock feats
    ...
    def forward(self, images):
        ...
        for i in range(batch_size):
            output = sample_model(images[i], self.model, self.sampler, up_ft_index=self.up_ft_index)
