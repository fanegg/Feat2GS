...
run_process() {
    local DATASET=$1 SCENE=$2 N_VIEW=$3 POINTMAP=$4 GPU_ID=$5 RENDER_TRAJ=$6
    
    local ERROR_LOG="${ERROR_DIR}/${METHOD}-${DATASET}-${SCENE}-${N_VIEW}-${POINTMAP}-${RENDER_TRAJ}.log"
    local LOCK_FILE="$LOCK_DIR/gpu_$GPU_ID.lock"
    touch $LOCK_FILE
    {
        echo "Running:"
            ...
            while [ $GPU_FOUND -eq 0 ]; do
                for GPU_ID in "${AVAILABLE_GPUS[@]}"; do
                    if is_gpu_free $GPU_ID; then
                        info=("GPU $GPU_ID is free, starting run:" "DATASET-${DATASET}" "SCENE-${SCENE}" "N_VIEW-${N_VIEW}" "POINTMAP-${POINTMAP}" "TRAJ-${TRAJ}")
                        max_length=$(printf "%s\n" "${info[@]}" | wc -L)
                        ...
                        run_process $DATASET $SCENE $N_VIEW $POINTMAP $GPU_ID $TRAJ &
                        sleep 3
